  ...
  try {
    ...
    if (!emailResult.success) {
      ...
      return res.status(201).json({
        ...
        verificationCode: verificationCode // Include code in development
      });
    }
...
const verifyEmail = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'SELECT id, email, first_name, verification_code_expires FROM users WHERE email = $1 AND verification_code = $2',
...
const resetPassword = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'SELECT id, email, first_name, reset_password_expires FROM users WHERE email = $1 AND reset_password_token = $2',
      [email, resetCode]
    );
